        ...
        try: # ---- try, except block if requests raises exception ----
            ...
            if re.match(regex_html, r.text) is not None: 
                ...
                for a_tag in a_tags:
                    ...
                    # if url > 0 and end with /, remove / from the last position
                    url = url[:-1] if len(url) > 0 and url[-1] == '/' else url

                    # ---- Continue if string contains URL pattern ----
                    if re.match(regex_url, url) is not None: 
                        ...
                        if links['links'].get(from_url) is None: # Create empty list for from_url (key)
                            links['links'].setdefault(from_url, []).append(url)
                        else: # add to list for from_url (key)
                            links['links'][from_url].append(url)
                            
                        links = scrape_links(url, for_depth - 1, links, start) # RECURSIVE STEP
        # cancel crawler with ctrl-c
        except KeyboardInterrupt:
            raise KeyboardInterrupt
        except Exception as e:
            # build excetion dictionary to console print
