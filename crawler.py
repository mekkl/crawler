...
def scrape_links(from_url, for_depth=0, all_links={}):
    '''
    ...
    regex = re.compile( r'^(?:http|ftp)s?://', re.IGNORECASE) # http:// or https://

    # -- Continue if depth >= 0 and if link doesn't already have been scraped -- 
    if for_depth >= 0 and links.get(from_url) is None: 
        try:
            ...
            for a_tag in tqdm(a_tags):
                ...
                if re.match(regex, url) is not None: # If regex matches (is a URL)
                    if links.get(from_url) is None: # Create empty list for from_url (key)
                        links.setdefault(from_url, []).append(url)
                    else: # add to list for from_url (key)
                        links[from_url].append(url)

                    links = scrape_links(url, for_depth - 1, links) # RECURSIVE STEP
        except:
            return links # return 
